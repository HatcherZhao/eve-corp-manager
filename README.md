...
面向网易 EVE Online 国服（Serenity）的 Web 军团管理项目。通过 CEO / 总监等有权限角色的国服授权，建立军团租户、识别游戏身份并逐步汇总军团业务数据。

**阶段：国服角色授权、军团租户、本站账号、权限刷新、权限树和成员权限管理已完成；资产、建筑、月矿、采矿账本和成员业务数据模块尚待实现。**
...
backend/ 与 frontend/ 已包含实际源码，无需另行克隆上游项目。先生成本地配置并启动 MySQL/Redis，再构建运行后端和前端。用户可注册或绑定 EVE 国服角色，并在「EVE 军团 → 军团工作台」查看授权状态、游戏权限及可访问能力。

## 开发路线图

工作台中“已具备访问资格”表示本站角色、国服授权 Scope 和游戏内角色权限均已满足，并不表示对应业务数据页面已经上线。

1. **权限体验收口**：将业务卡片明确标为“开发中”，保留授权、刷新、权限树和成员权限管理等已完成能力。
2. **军团人员**：接入成员名册、游戏身份、入团/离团状态、管理层权限及同步记录，为后续人员关联提供基础。
3. **军团资产**：实现资产拉取、分页同步、快照保存、机库/容器层级、位置与数量查询，以及筛选导出。
4. **建筑设施与月矿计划**：展示建筑状态、服务、燃料到期，以及月矿提取、到达、自然碎裂时间线和提醒。
5. **采矿账本**：按成员、矿种、日期、地点汇总观察者账本，提供明细追溯与导出。
6. **运营与自动化**：增加定时同步、数据新鲜度、授权到期与异常提醒、操作审计及军团运营看板。
...
登录交互采用用户确认的分步国服授权与回调网址粘贴方案，详见登录方案。后续业务模块须使用当前授权角色和军团的真实数据，不能借用 Swagger UI、Pyfa 或其他第三方应用的 client_id / callback。
